...
每篇 800–2500 字，含真实感错误码、IP/端口、操作步骤与章节标题（同时检验语义切块与引用渲染）。
...
- [ ] 1. evals/ 骨架：dataset.py + validate 脚本 + 离线单测
- [ ] 2. 语料 24 篇 + 评测集 120+ 条撰写 + 校验通过 + 复核记录
- [ ] 3. metrics.py + 单测
- [ ] 4. pipelines.py（旧版重建 + 新版适配）+ 小规模烟测
