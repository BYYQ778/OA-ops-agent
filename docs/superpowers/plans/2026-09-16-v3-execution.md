...
- [x] 1. 工程基线：可复现依赖、pytest、Ruff、Pyright、pre-commit、CI、渐进路由拆分。首批覆盖日志规则、图谱、巡检解析、SQLite、真实 API。核心覆盖率目标 80%，全项目目标 60%；必须说明统计范围，不用排除业务代码冒充达标。**（2026-09-17：本地门槛达成 + 远端 CI 首跑全绿；2026-09-19 桌面 GUI 与绿色版随合并重建后重验通过，已勾选）**
- [x] 2. RAG：BM25 + Dense + RRF、可选重排序、结构化路由、证据引用、低置信度拒答。旧向量库需保留，换 Embedding 时新建版本化索引并评测后切换。**（2026-09-17 主体完成：712 passed + 全链路运行验证；2026-09-18 阈值校准完成，分层证据门冻结 0.89/10.25 ∥ 0.60/7.75；未更换 Embedding，版本化索引条件未触发，已勾选）**
- [x] 3. 评测：至少 100 条标注问答，冻结留出集；同一数据/硬件比较旧版与新版 Recall@5、MRR、NDCG、引用、拒答、延迟。目标是验收条件，不是预先宣称的成果。**（2026-09-18 完成：24 篇语料 + 128 条标注集（dev88/holdout40）；Recall@5 74.1%→94.4%、MRR 0.609→0.807、引用覆盖率→98.1%、无证据拒答 0→75%；报告 docs/reports/rag-eval-report.md）**
...
- [x] 6. 交付：经过验证的 Windows 绿色版、README/架构/安全/API/评测文档、演示素材、简历。无运行或测量证据的指标不能写成已达成。**（2026-09-19 完成并发布：绿色版重建实测（zip 1.06GB / 8234 文件）+ README/ARCHITECTURE/CHANGELOG/API/SECURITY 文档 + 演示素材（GIF/视频/截图）+ 简历与面试材料；GitHub Release v3.0.0 附件 zip + SHA256SUMS）**
...
- 2026-09-19（第 6 周 · 交付收口）: release/v3.0.0（worktree 6）完成并发布 —— PR #1~#5 逐级
  合并（1ae1ec4→685cfef）+ #6 交付 PR（398eefb）全部并入 master，远端由 v2.4.1 追平至 v3.0.0；
  README 重写（前 30 秒六要素 + 实测数字）、ARCHITECTURE/CHANGELOG/API/SECURITY 文档、演示素材
  （GIF 4.3MB + 74 秒视频 + 截图×3 + 社交预览图，均为实机录制，含 Ollama 本地模型真实问答）。
- 绿色版：PyInstaller 重建（约 15 分钟）→ app/ 外置同步 → zip 1.06GB / 8234 文件 → 冻结冒烟
  （就绪 14 秒；导入/检索/根因/指标/首页全通过）→ GUI 双击实测（VLM 核验窗口渲染）→ 用户历史
  数据兼容（3 文档 + KG 89 实体/639 边）；GitHub Release v3.0.0 发布（附件 zip + SHA256SUMS，
  sha256 a041d9c4fc83b0c0dc58f7ca09259065e9534edcdad0ad3bf6371e1df3761bb8）。
- 发布前检查：927 passed + Ruff/Pyright 全绿；master CI 全绿（quality + docker-core）；
  敏感信息扫描 0 泄漏；pip-audit 1 条已知通告（pypdf2 3.0.1，修复需迁移 pypdf → 列入 v3.0.1）。
  六周计划六阶段全部完成。
