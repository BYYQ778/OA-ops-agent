...
- [ ] 5. 可观测性/安全：本地结构化日志、OpenTelemetry、Prometheus、可选 Langfuse、Session/RBAC、密钥隔离。桌面启动方式兼容；未完成鉴权前不得公开部署。**（2026-09-18 feat/observability-security 本地完成：结构化日志/指标/OTel/Session-RBAC/密钥隔离/启动门禁；严格模式 E2E 15/15；推送与远端 CI 待做，故不勾选）**
...
- 推送与远端 CI 待做，故不勾选。
- 下一步: 推送本分支 → 第 6 周交付（release/v3.0.0：绿色版重建 + README/演示/简历材料）。
